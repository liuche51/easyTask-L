...
public class NettyClientTest {
        ...
        try {
           ...
           // AnnularQueue.getInstance().setConfig(config);
            while (true) {
                ...
                //这种异步消息回调方法，没法获取服务器返回的结果信息，只能知道是否完成异步通信的发送，且不会等待服务端返回结果。有点坑
                future.addListener(new GenericFutureListener<Future<? super Void>>() {
                    @Override
                    public void operationComplete(Future<? super Void> future) throws Exception {
                        if (future.isSuccess()) {
                            System.out.println("服务器返回完成！start=" + start);
                        }

                    }
                });
                System.out.println("消息已发送，等待回调！");
                Thread.sleep(5000);
            }
    ...
    @Test
    public void syncObjectNodeClockDiffer() {
        try {
            long start = System.currentTimeMillis();
            Thread.sleep(2000);
            long nodetime=System.currentTimeMillis()-4000l;
            Thread.sleep(2000);
            long end=System.currentTimeMillis();
            long differ=end-(end-start)/2-nodetime;
